...
const InspectionForm = (props) => {
    ...
    const[ProductCodeInfoList, SetProductCodeInfoList] = useState([])
    const [ProductInputModalVisible, SetProductInputModalVisible] = useState(false)

    const FunctionalityCheckParameters = ["Rocking", "Wobblig", "Shipment"]
    const SpecialFeaturesCheckParameters = ["Fabrication", "Aesthetic look"]
    const VisualCheckParameters = ["Fabrication", "Aesthetic look", "Beauty"]
    const StabilityCheckParameters = ["Static Load Test"]
    const OnsiteCheckParameters = ["Shipping Marks"]
        ...
        >
            ...
            if(SelectedOrderInfo.inspectionID != 0 &&  ! DataLoaded)
                ...
                >
                    ...
                    >
                        {/* <TouchableOpacity
                            id="ProductCodeModalHeader"
                            style={{height: 0.07*screenHeight, width: screenWidth, backgroundColor: Colors.primaryColor}}
                            onPress={() => {SetProductInputModalVisible(false)}}
                        >
                            <View style={{flexDirection: "row", justifyContent: "flex-start", top: 0.022*screenHeight}}>
                                <Text style={{color: Colors.accentColor, marginHorizontal: 10, fontSize: 17}}>Back</Text>
                                <Text style={{color: Colors.accentColor, marginHorizontal: "20%", fontSize: 18, fontWeight: "bold"}}>Add Product</Text>
                            </View>
                        </TouchableOpacity> */}


                    <View id="PO Wise Quantity" style={{borderColor: Colors.primaryColor, borderWidth: 0, borderRadius: 5, justifyContent: "center", alignItems: "center"}}>
                    ...
                    <FloatingLabelInput
                            ...
                            containerStyles={{...styles.input, width: Dimensions.get("window").width / 1.2, height: 45, backgroundColor: "white"}}
                            inputStyles={{fontWeight: "bold", fontSize: 18, color: Colors.primaryColor}}
                            maxLength={50}
                            // editable={false}
                            value={CurrentProduct}
                            onChangeText = {(newChangedText) => SetCurrentProduct(newChangedText)}
                    />
            ...
            {(() => {
                    if(IsPartInspection == "undecided" || InspectionTypeSelected)
                        return (
                            <View style={{borderColor: Colors.primaryColor, borderRadius: 5, marginTop: 10, borderWidth: 1, width: "80%"}}>
            ...
            <View id="cartonInfo" style={{flexDirection: "row", alignSelf: "center"}}>
                    ...
                    />
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                ...
                                newProductInfoObject["OrderQuantity"] = OrderQuantity
                                newProductInfoObject["OfferedQuantity"] = OfferedQuantity
                                newProductInfoObject["SampleSize"] = SampleSize
                                newProductInfoObject["ExcessQuantity"] = ExcessQuantity
                                newProductInfoObject["IsPartInspection"] = IsPartInspection
                                newProductInfoObject["PackedQuantity"] = PackedQuantity
                                newProductInfoObject["TotalCartons"] = TotalCartons
                                newProductInfoObject["CartonSampleSize"] = CartonSampleSize
                                newProductInfoObject["CartonSelected"] = CartonSelected.toString()

                                newProductInfoObject["Functionality Checks"] = {}
                                newProductInfoObject["Special Features"] = {}
                                newProductInfoObject["Visual Checks"] = {}
                                newProductInfoObject["Stability Checks"] = {}
                                newProductInfoObject["Onsite Checks"] = {}

                                for (var checklist of FunctionalityCheckParameters)
                                    newProductInfoObject["Functionality Checks"][checklist] = "undecided"

                                for (var checklist of SpecialFeaturesCheckParameters)
                                    newProductInfoObject["Special Features"][checklist] = "undecided"

                                for (var checklist of VisualCheckParameters)
                                    newProductInfoObject["Visual Checks"][checklist] = "undecided"

                                for (var checklist of StabilityCheckParameters)
                                    newProductInfoObject["Stability Checks"][checklist] = "undecided"

                                for (var checklist of OnsiteCheckParameters)
                                    newProductInfoObject["Onsite Checks"][checklist] = "undecided"
                                



                                var newProductCodeInfoList = [newProductInfoObject].concat(ProductCodeInfoList)
                                console.log("##### Product Code Info List ######")
                                console.log(newProductCodeInfoList)

                                SetProductCodeInfoList(newProductCodeInfoList)
                                SetCurrentProduct("")
                                SetOfferedQuantity("")
                                // SetOrderQuantity("")
                                SetSampleSize("")
              ...
              //On text change listner on the searchable input
                ...
                <View id ="ProductsTable" style={{marginVertical: 10, width: "99%", marginLeft: "1%"}}>
                    {(() => {
                    ...
                    <FlatList 
                        data={ProductCodeInfoList}
                        keyExtractor={(ProductObject) => ProductObject.ProductCode}
                        style={{}}
                        renderItem = {({item}) => {
                        return (
                    ...
                    />
                     <TouchableOpacity
                            style={{ ...styles.openButton, backgroundColor: Colors.primaryColor, marginHorizontal: 10, width: "50%"}}
                            onPress={() => SetProductInputModalVisible(true)}
                            disabled={SelectedOrderInfo.inspectionID != 0}
                        >
                          <Text style={{...styles.textStyle, color: Colors.accentColor}}>+ Add Product</Text>
                        </TouchableOpacity>
                </View>
                ...
                <FloatingLabelInput
                        ...
                        inputStyles={{color: "grey", fontWeight: "bold"}}
                        containerStyles={{...styles.input, height: 50, justifyContent: "center", marginTop: 2, marginBottom: 5}}
...
{/* ####################################################################################################################################################### */}
                ...
                renderItem = {({item}) => {
                    ...
                    <FlatList 
                        data={(() => {
                            var rowList = []
                            for ( var ProductObject of ProductCodeInfoList)
                            {
                                
                                for(var checkParameter of Object.keys(ProductObject[CheckType]))
                                {   var rowObject = {}
                                    rowObject["ProductCode"] = ProductObject.ProductCode
                                    rowObject[CheckType] = checkParameter
                                    rowObject["Result"] = ProductObject[CheckType].checkParameter
                        ...
                        keyExtractor={(rowObject) => `${rowObject.ProductCode} ${rowObject[CheckType]}`}
                        style={{}}
                        renderItem = {({item}) => {
                        return (

                            <View style={{flexDirection: "row", alignContent: "center", alignItems: "center",  height: 35, borderWidth: 1, justifyContent: "flex-start", borderRadius: 5}}>
                                <Text numberOfLines={2} numberOfLines={2} style={{textAlign: "justify", marginLeft: "3%", fontSize: 10, fontWeight: "bold", color: "grey", textAlign: 'center', width: "25%"}}>{item.ProductCode}</Text>
                                <Text numberOfLines={2} style={{textAlign: "justify", marginLeft: "1%", fontSize: 10, fontWeight: "bold", color: "grey"}}>{item[CheckType]}</Text>
                                <Text numberOfLines={2} style={{textAlign: "justify", marginLeft: "12%", fontSize: 10, fontWeight: "bold", color: "grey"}}>{item.Result}</Text>
                                ...
                                <SwitchSelector
                                    ...
                                    initial={-1}
                                    onPress={(value) => {
                                        var newProductCodeInfoList = [].concat(ProductCodeInfoList)
                                        for(var ProductCodeObject of newProductCodeInfoList)
                                        {   
                                            if(ProductCodeObject.ProductCode == item.ProductCode)
                                                ProductCodeObject[CheckType][item[CheckType]] = value
                                        }

                                        SetProductCodeInfoList(newProductCodeInfoList)
                                        console.log("######## New product Code info list #######")
                                        console.log(newProductCodeInfoList)
                                    }}
                                ...
                                <View id="Image icon" style={{flexDirection: "row", justifyContent: "flex-start", right: 5, position: 'absolute'}}>
                                        <Icon
                                            name='ios-camera'
                                            type='ionicon'
                                            color={Colors.primaryColor}
